...
# the dell keyword can remove a value at a specific index, and
...
# del with no index removed entirely
def simple_del_none():
    del sizes
    for value in sizes:
        print(value)
...
# adding items to a set that can be done by add() and update()
# add method
def simple_set_add():
...
# discard will remove without generating an error if the value is not in the list.
...
# the value however can be a duplicate. Defined using {}
